...
then
    ...
    export PGPASSWORD=$user_pg_pass;psql -h $db_host -U $user_pg -d $db_name -f data/gas.sql &> var/log/install_db.log
    if $insert_minimal_data
        then
            echo "Insertion des données minimales dans la base de données..."
            export PGPASSWORD=$user_pg_pass;psql -h $db_host -U $user_pg -d $db_name -f data/gas-data.sql &>> var/log/install_db.log
    fi
fi
